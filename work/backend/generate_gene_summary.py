...
#     print(cds_results['gene_name'])
    ...
    for index, row in haplotype_calls['df_all_haplotypes'].iterrows():
        # Check and add sample IDs to the set for each rule
        if '-' in row['nucleotide_haplotype']:
            a.add(row.name)
        if ',' in row['aa_haplotype']:
            b.add(row.name)
        if '!' in row['ns_changes']:
            c.add(row.name)

    removed_missing = len(a)
    removed_het_calls = len(b -a)
    removed_stop_codon = len(c - b - a)
