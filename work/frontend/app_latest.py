...
base_path = "/nfs/team112_data03/personal/nw20/gitlab/malariagen/gsp/mutation-discovery-app/work/array_job_all"

# Disgusting hack. Let's fix this upstream at the backend level
file_directory = {
    f: (f.replace("prep_plot_data_", "").replace(".pkl.xz", "") if f.count("PF3D7") == 1 else f.replace("prep_plot_data_", "").replace(".pkl.xz", "").partition("_PF3D7")[0])
    for f in os.listdir(base_path)
    if f.endswith(".pkl.xz")
    # & ~f.endswith("_VAR.pkl.xz") & ~f.endswith("_RIF.pkl.xz")
}

# Create a list of unique gene names and sort it alphabetically
gene_names = sorted(set(file_directory.values()))
gene_ids = [gene.rpartition("_")[0] if gene.count("_") == 2 else gene for gene in gene_names]
default_gene_id_index = gene_ids.index("PF3D7_0523000")
...
if True in gene_id_query_array:
    ...
    gene_id_query_index = default_gene_id_index
else:
    st.error(f'Query gene, "{gene_id_query["gene_id"][0]}" is not valid')
    gene_id_query_index = default_gene_id_index

def selectbox_with_query_storage(label, options, query_param_name, default_value, **kwargs):
    ...
    if len(current_value_params) == 0:
        current_value_params = [default_value]
    
    query_value = None
    if len(current_value_params) == 1:
        query_value = current_value_params[0]
...
selected_gene_name = selectbox_with_query_storage("Select gene name", options = gene_names, query_param_name = "gene_id", default_value = "PF3D7_0523000_MDR1")

selected_gene_id = (selected_gene_name if selected_gene_name.count("_") == 1 else selected_gene_name.rpartition("_")[0])
st.experimental_set_query_params(gene_id = selected_gene_id)
selected_gene_plasmodb_url = f'https://plasmodb.org/plasmo/app/record/gene/{selected_gene_id}'

if ("VAR" in selected_gene_name) | ("RIF" in selected_gene_name):
...
upset_plot_height = 1.5 + len(set(np.concatenate(df_haplotypes.ns_changes_list))) / 50
