...
from src.utils import cache_load_population_colours, cache_load_population_colours_kelch, _cache_load_utility_mappers
import io
...
buffer = io.BytesIO()
def generate_haplotype_plot_kelch(df_haplotypes, background_ns_changes, min_samples, sample_count_mode):
    """Main function called in main.py to generate and present haplotype plot"""
    ...
    # df_haplotypes['cum_proportion'] = df_haplotypes['Total'].cumsum() / total_samples 
    df_haplotypes_set = df_haplotypes.loc[df_haplotypes['Total'] >= min_samples] 
    df_haplotypes_set = df_haplotypes_set.copy()
    df_haplotypes_set['ns_changes'] = df_haplotypes_set['ns_changes'].replace('', '3D7 REF')
    # Sort haplotypes by total number of samples
    df_haplotypes_set.sort_values('Total', inplace=True, ascending=False)
    ...
    mutations = mutations.loc[mutations != '']
    mutations_np_ref = mutations.apply(lambda x: x[1:])
    aa = mutations_np_ref.apply(lambda x: int(x[:-1]) if x[:-1].isdigit() else print(f"Non-digit character: {x[:-1]}"))
    df_mutations_set = pd.DataFrame(
        {
            'mutation': mutations.values,
            'aa': aa.values,
    ...
    for column_num in np.arange(1, df_haplotypes_set.shape[1] - 3): # fixed value, change if you added a column
        pop = df_haplotypes_set.columns[column_num]
        proportion = ((df_haplotypes_set[df_haplotypes_set.columns[column_num]] / df_haplotypes_set['Total']).round(3))*100
    ...
    fig.add_traces(bars, rows=2, cols=1)
    fig.update_layout(barmode='stack', legend=dict(x=1, y=1 - 2 / (5 + upset_plot_height), traceorder='reversed'), margin=dict(l=40, r=40, t=10, b=20))
    # 2 is the row height ratio of the first plot, so this calculation places the legend right below the first plot
    ...
    # Define color map for each mut_status category
    mut_status_colors = {
    'validated': 'green',  # Replace with your desired color
    'candidate': 'blue',   # Replace with your desired color
    'not associated': 'grey'  # Replace with your desired color
    # Add more categories and colors as needed
    }
    df_haplotypes_set['mut_status_color'] = df_haplotypes_set['mut_status'].apply(lambda x: mut_status_colors.get(x, 'gray'))

    legend_sentence = "Points are colored by mutation status: <span style='color:green'>Validated</span>, <span style='color:blue'>Candidate</span>, <span style='color:grey'>Not associated</span>."
    ...
    for ix, row in df_haplotypes_set.iterrows():
        if row['ns_changes_list'][0] != '':
            indexes = df_mutations_set.loc[row['ns_changes_list'], 'index'].values
            mut_status_values = row['mut_status']
            fig.add_traces(go.Scatter(
                x = [i] * len(indexes),
                y = indexes,
                showlegend=False,
            ...
            fig.add_traces(go.Scatter(
                ...
                mode='lines+markers',
                hovertemplate='%{y}<extra></extra>',
                marker=dict(size=marker_size, color= row['mut_status_color']),
            ),
                           rows = 3, cols = 1)
        i = i + 1
    # Add the dummy traces to create the legend
    #fig.add_traces(legend_traces, rows=3, cols=1)
    ...
    fig.update_yaxes(showgrid = True, zeroline = False, gridcolor='rgba(0, 0, 0, 0.15)', 
                     ...
                     ticktext=df_mutations_set.reset_index().mutation,
                     row = 3, col = 1)
    fig.update_layout(hovermode = 'closest',  legend=dict(y=0.81),)
    fig.add_annotation(
    go.layout.Annotation(
        xref='paper',
        yref='paper',
        x=0,
        y=-0.01,  # Adjust the y-coordinate to control the vertical position
        text=legend_sentence,
        showarrow=False,
    ...
    data=buffer,
    file_name="haplotypes_figure.png",
