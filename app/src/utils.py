...
def _cache_load_sample_metadata():
    return pd.read_csv(f'{DATAPACK_BASE_PATH}/auxiliary/Pv5_public_metadata_20260811.tsv', sep = "\t", low_memory = False)
...
def cache_load_population_colours():
    """Pv5 population colour palette. Caches the objects when first loaded"""
    population_colours = collections.OrderedDict()
    population_colours["LA"]    = "#4daf4a" 
    population_colours["AF-ETH"]    = "#e31a1c"
    population_colours["AS-W"]    = "#984ea3"
    ...
    population_colours["Unassigned"]      = "#D3D3D3"
...
@st.dialog("Pv5 is here! 🥳")
def _present_cookie_banner():
    tab1, tab2 = st.tabs(["Welcome", "Cookie details"])

    tab1.markdown("We now use [**MalariaGEN Pv5**], which means that the Pv-HaploAtlas contains data from **1,265** QC-passed samples. Happy haplotype hunting!")
