...
run `python3 -m streamlit run app_nina_edits_021123.py` in command line (you may have to pip install streamlit first)
...
with st.sidebar:
    ...
    st.markdown("**4. Abacus plot** - click on a haplotype to view its frequency across first-level administrative divisions and years.")
...
with st.expander("Click to edit plot settings"):
    ...
    sample_count_mode = st.radio("Select a mode for Plot 1:", ["Cumulative statistics", "Y-axis log scale"], index=0)
...
df_haplotypes_set['ns_changes'] = df_haplotypes_set['ns_changes'].replace('', 'REF') # Fill in haplotype with no ns_changes as 'REF'
...
if sample_count_mode == "Cumulative statistics":
...
for column_num in np.arange(2, df_haplotypes_set.shape[1] - 4):
    ...
    proportion = (df_haplotypes_set[df_haplotypes_set.columns[column_num]] / df_haplotypes_set['Total']).round(3)
    
    # Attempt to only display pops with >0 in the trace - not working thus far
