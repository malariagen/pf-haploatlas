"""
bsub -J[1-100] -o generate_gene_summary-%J.output -e generate_gene_summary-%J.error -M200000 -R "select[mem>200000] rusage[mem=200000] span[hosts=1]" -G team342 'python3 generate_gene_summary.py ${LSB_JOBINDEX}'
"""
...
n_jobs = 100
n_genes = len(Pf3D7_genes_list)
chunk_size = round(n_genes / n_jobs)
print("Chunk size:", chunk_size)

index_list = [[chunk_size * v, chunk_size * (v+1)] for v in np.arange(n_jobs + 1)]
job_indexes = np.arange(*index_list[LSB_JOBINDEX])
...
#                     print(len(ref_aa_haplotype), len(aa_1))
    ...
    call_haps_pickle_fn = f'call_haplotypes_data_{gene_id}.pkl.xz'
...
#     print(cds_results['gene_name'])
    ...
    prep_plot_pickle_fn = f'prep_plot_data_{gene_id}.pkl.xz'
...
for job_index in job_indexes:
    prepare_plot_data(gene_id=Pf3D7_genes_list[job_index])